...
export default async (req, res) => {
  ...
  try {
    ...
    // If z, x, y are provided, proxy the actual tile
    if (z && x && y) {
      const tileUrl = mapId.urlFormat.replace('{z}', z).replace('{x}', x).replace('{y}', y);
      ...
      try {
        ...
        console.error('Tile proxy error:', tileError);
        res.status(500).json({ error: `Tile proxy failed: ${tileError.message}` });
      }
    } else {
